...
def Employee_Login(request):
	if request.method == "POST":
		...
		if userDetails.objects.filter(username=C_name, password=C_password).exists():
			...
			request.session['UserType'] = C_name.lower()
        ...
        if userDetails.objects.filter(username=username).exists():
            ...
            while True:
                ...
                # screenshots will be taken
                    ...
                    path = os.path.join(settings.BASE_DIR, 'media', 'user_images')
...
def Attendance(request):
	if face_recognition is None:
		...
		_messages.error(request, 'face_recognition (and dlib) is not installed. See README_WINDOWS_INSTALL.md to install dlib.')
		return redirect('/')
	...
	while True:
		...
		for encodeFace,faceLoc in zip(encodesCurrentFrame,facesCurrentFrame):
			matches = face_recognition.compare_faces(encodeListKnown,encodeFace, tolerance=0.9)
			faceDis = face_recognition.face_distance(encodeListKnown,encodeFace)

			matchIndex = np.argmin(faceDis)

			print(f"Face detected, distances: {faceDis}, matchIndex: {matchIndex}, matches: {matches[matchIndex]}")

			if matches[matchIndex]:
				name = personName[matchIndex].lower()
				...
				if attendance.objects.filter(name = name,date=dStr).exists():
					...
					messages.info(request, f'Attendance already marked for {name} today')
					processed = True
					break
				# else:
				time_now = datetime.now()
				...
				processed = True				
				break  # Stop after marking attendance
		if processed:
			break
...
def View_Attendance_User(request):
	if 'UserType' not in request.session or request.session.get('type_id') != 'User':
		messages.error(request, 'Access denied. Please login as an employee.')
		return redirect('/Employee_Login/')
	C_name=request.session['UserType']
	details = attendance.objects.filter(name = C_name)
	print(f"View_Attendance_User: UserType={C_name}, Records found={len(details)}")
	for d in details:
		print(f"  - {d.name} {d.date} {d.time}")
